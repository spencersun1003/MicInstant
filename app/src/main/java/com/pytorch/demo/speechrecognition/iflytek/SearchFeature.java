...
import com.pytorch.demo.speechrecognition.MainActivity;
...
public class SearchFeature {
    ...
    //构造函数,为成员变量赋值
    public SearchFeature(String requestUrl,String APPID,String apiSecret,String apiKey,String GroupID,String FeatureID,String AUDIO_PATH){
    ...
    //提供给主函数调用的方法
    public static boolean doSearchFeature(String requestUrl,String APPID,String apiSecret,String apiKey,String GroupID,String FeatureID,String AUDIO_PATH) {
        SearchFeature searchFeature = new SearchFeature(requestUrl, APPID, apiSecret, apiKey,GroupID,FeatureID, AUDIO_PATH);
        try {
            ...
            JSONArray resultArray =jsonObject.getJSONArray("scoreList");
            //boolean FoundVoicePrint=false;
            for (int i=1;i<resultArray.size();i++){
                if (resultArray.getJSONObject(i).getFloat("score")> MainActivity.VoicePrint_ScoreThreshold){
                    System.out.println("doSearchFeature: from list, score=="+String.valueOf(resultArray.getJSONObject(i).getFloat("score")));
                    return true;
                }
            }
            return false;
